...
/******************************************************************************
 * Class Name : AprioriCalculation Purpose : generate Apriori itemsets
 *****************************************************************************/
class AprioriCalculation {
	...
	/************************************************************************
	 * Method Name : getInput Purpose : get user input from System.in Parameters
	 * : None Return : String value of the users input
	 *************************************************************************/
	...
	/************************************************************************
	 * Method Name : getConfig Purpose : get the configuration information
	 * (config filename, transaction filename) : configFile and transaFile will
	 * be change appropriately Parameters : None Return : None
	 *************************************************************************/
	private void getConfig() {
		...
		if (input.compareToIgnoreCase("c") == 0) {
			...
			System.out
					.print("Enter the separating character(s) for items (return for '"
							+ itemSep + "'): ");
			input = getInput();
			if (input.compareToIgnoreCase("") != 0)
				itemSep = input;
		...
		try {
			...
			System.out
					.print("Enter 'y' to change the value each row recognizes as a '1':");
	...
	private void calculateFrequentItemsets(int n) {
		...
		boolean match; // whether the transaction has all the items in an
						// itemset
		boolean trans[] = new boolean[numItems]; // array to hold a transaction
			...
			for (int i = 0; i < numTransactions; i++) {
				...
				for (int j = 0; j < numItems; j++) {
					trans[j] = (stFile.nextToken().compareToIgnoreCase(
							oneVal[j]) == 0); // if it is not a 0, assign the
												// value to true
				}
